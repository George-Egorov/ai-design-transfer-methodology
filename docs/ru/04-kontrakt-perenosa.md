...
Контракт BRIDGE — расширенная версионируемая запись, независимая от целевой платформы и связывающая макет с реализацией и проверкой. Открывайте эту страницу для интеграции и инструментов, а не как первое чтение дизайнера. Структурированная оболочка и [JSON Schema](../../validator/bridge.schema.json) могут меняться до 1.0, поэтому в каждой передаче фиксируйте `contractVersion`, `methodologyVersion` и `rulesVersion`.
